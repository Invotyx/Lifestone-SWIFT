...
//  LifeStone
//
//  Created by Wajih Invotyx on 26/07/2019.
...
public class WajihTextField: UITextField {
    ...
    @IBInspectable open var titleFont: UIFont = UIFont.systemFont(ofSize: 17) {
        didSet {
            if let title = self.titleLabel {
    ...
    @IBInspectable open var lineHeight: CGFloat = 2 {
        didSet {
    ...
    public override func layoutSubviews() {
        ...
        self.addTarget(self, action: #selector(WajihTextField.editingChanged), for: .editingChanged)//.editingChanged
    }
    ...
    private func titleLabelRectForBounds(_ bounds: CGRect) -> CGRect {
        ...
        return CGRect(x: 16, y: -self.titleHeight, width: bounds.size.width, height: titleHeight)
    }
    
    private func lineViewRectForBounds(_ bounds: CGRect) -> CGRect {
//        let lineHeight = self.lineHeight
        return CGRect(x: 0, y: -self.titleHeight+12, width: bounds.size.width, height: 1);
